...
function parseArgs(argv = []) {
  ...
  if (options.command === 'converge' && options.name.startsWith('-')) {
    options.name = '';
    startIndex = 1;
  }
...
export function planGuestRuntimeDefaults({
  ...
  if (isToolIdAvailable(normalizedGuestSelection?.selectedTool, nextTools)) {
    selection = normalizeUiRuntimeSelection({
      ...normalizedGuestSelection,
      updatedAt: new Date().toISOString(),
    });
  } else if (isToolIdAvailable(normalizedOwnerSelection?.selectedTool, nextTools)) {
    selection = normalizeUiRuntimeSelection({
      ...normalizedOwnerSelection,
      updatedAt: new Date().toISOString(),
    });
  } else {
    selection = normalizeUiRuntimeSelection({
      selectedTool: 'codex',
      selectedModel: trimString(normalizedGuestSelection?.selectedModel)
        || trimString(normalizedOwnerSelection?.selectedModel)
        || detectedModel
        || 'gpt-5.4',
      selectedEffort: trimString(normalizedGuestSelection?.selectedEffort)
        || trimString(normalizedOwnerSelection?.selectedEffort)
        || 'high',
      thinkingEnabled: normalizedGuestSelection?.thinkingEnabled === true,
      reasoningKind: trimString(normalizedGuestSelection?.reasoningKind)
        || trimString(normalizedOwnerSelection?.reasoningKind)
        || 'enum',
      updatedAt: new Date().toISOString(),
    });
  }
