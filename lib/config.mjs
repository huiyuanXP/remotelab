...
export const QUICK_REPLIES_FILE = join(configDir, 'quick-replies.json');
export const INTERRUPTED_SESSIONS_FILE = join(configDir, 'interrupted-sessions.json');
export const REPORT_TO_FILE = join(configDir, 'report-to-watchers.json');
export const SESSION_LABELS_FILE = join(configDir, 'session-labels.json');
export const UI_SETTINGS_FILE = join(configDir, 'ui-settings.json');

// Auto-compact: when context usage exceeds this fraction of the window, auto-compact triggers
export const AUTO_COMPACT_THRESHOLD = parseFloat(process.env.AUTO_COMPACT_THRESHOLD) || 0.80;
export const CONTEXT_WINDOW_SIZE = parseInt(process.env.CONTEXT_WINDOW_SIZE, 10) || 200000;
